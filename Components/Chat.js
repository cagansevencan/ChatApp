...
class Chat extends Component {
    ...
    render() {
        return (this.props.activeUser && <Fragment>
            ...
            <div className="px-4 pb-4 w-100 d-flex flex-row flex-wrap
             align-items-start align-content-start position-relative"
                 style={{height: 'calc(100% - 180px)', overflowY: 'scroll' }}>
                {this.state.chats.map((chat, index) => {
                    const prev = Math.max(0, index-1);
                    const prevChat = this.state.chats[prev];
                    const position = chat.user === this.props.activeUser ? "right" : "left";

                    const isFirst = prev === index;
                    const inSequence = chat.user === prevChat.user;
                    const hasDelay = Math.ceil((chat.timestamp - prevChat.time) / (1000 * 60)) > 1;
